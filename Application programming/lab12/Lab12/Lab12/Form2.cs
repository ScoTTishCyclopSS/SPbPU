            ...
            try
            {
                ...
                using (FileStream fs = new FileStream(file, FileMode.Open))
                {
                    while (fs.Position < fs.Length)
                    {
                        ...
                        dataGridView1.Rows.Add(Encoding.GetEncoding(866).GetString(org.name),
                            Encoding.GetEncoding(866).GetString(org.address), org.tel,
                            Encoding.GetEncoding(866).GetString(org.contact));
                    }
                }
            }
            catch
